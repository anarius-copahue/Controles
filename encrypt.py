...
from encrypt import encrypt_file  # 🔒 Importamos tu función original de encriptación

# Leer credenciales y clave de cifrado desde las variables de entorno de GitHub Actions
...
ENCRYPTION_KEY = os.environ.get("ENCRYPTION_KEY") # Nueva variable clave

DOWNLOAD_DIR = os.path.join(os.getcwd(), "descargas")

# Definimos los reportes y cómo se llamarán sus versiones encriptadas finales
REPORTES_DISPRO = {
...
def delete_previous_file(file_name):
    ...
    if os.path.exists(file_path):
        os.remove(file_path)
    # También borramos el encriptado previo local si existe para no duplicar datos corruptos
...
def scrape_data():
    ...
    try:
        ...
        # 🔒 ENCRIPTRACIÓN DE REPORTES DISPROFARMA
        print("Encriptando archivos descargados de Disprofarma...")
        for csv_nombre, ruta_destino_encriptado in REPORTES_DISPRO.items():
            ...
            if os.path.exists(ruta_csv_plano):
                ...
                os.remove(ruta_csv_plano) # Borramos el plano por seguridad
        ...
        os.remove(ruta_plana_shopify) # Limpiamos el plano de la memoria
